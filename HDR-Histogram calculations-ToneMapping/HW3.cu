...
void loadImageHDR(const std::string &filename, float **imgPtr, size_t *numRows, size_t *numCols)
{
  size_t rows = *numRows;
  size_t cols = *numCols;
  ...
  cout <<" cols: " << cols<< "\n";
  int ch = 4;
  float* image_rgba = *imgPtr;
  image_rgba = (float*) malloc(rows*cols*3*sizeof(float));
  //*imgPtr = (float*) malloc(rows*cols*3*sizeof(float));
  int *arr;
  arr = (int*) malloc(rows*cols*4*sizeof(int));
  string a;
  cout <<"6 \n";
  const char * c = (filename).c_str();
  cout << "7 \n";
  fstream file(c, ios::in);
  if(!file.is_open()){
    cout << "File not found!\n";
            //return 1;
  }
  ...
  for (int i = 0; i<rows*cols; i++){
    image_rgba[3*i] =  arr[4*i+2];
    image_rgba[3*i+1] =  arr[4*i+1];
    image_rgba[3*i+2] =  arr[4*i+0];
  }
  
  *imgPtr = image_rgba;
  cout << "10 \n"<< image_rgba[0] << " "<< (*imgPtr)[0] << " "<< arr[2]<< "\n"; 
...
void preProcess(float** d_luminance, unsigned int** d_cdf,
  ...
  loadImageHDR(filename, &imgPtr, numRows, numCols);
  cout << "4 \n";
  numRows__ = *numRows;
  numCols__ = *numCols;
  cout << "5 \n";
...
void fromRGBToText(float* h_grey, size_t rows, size_t cols){
  ofstream outputFile("grey7.txt");
  for(int i=0; i<cols; i++){
    for(int j=0; j<rows; j++){
      if((i+j)<rows+cols-2){
        outputFile << (float) h_grey[3*(i*rows+j)] << "," <<(float) h_grey[3*(i*rows+j)+1] << "," <<(float) h_grey[3*(i*rows+j)+2] <<"," ;}
        else outputFile << (float) h_grey[3*(i*rows+j)] << "," <<(float) h_grey[3*(i*rows+j)+1] << "," <<(float) h_grey[3*(i*rows+j)+2]  ;
    }
  }
...
void postProcess(size_t numRows, size_t numCols,
  ...
  for (int i = 0; i < numPixels; ++i) {
    imageHDR[3 * i + 2] = h_blue[i];
    imageHDR[3 * i + 1] = h_green[i];
    imageHDR[3 * i + 0] = h_red[i];
    cout<<h_blue[i]<<"\n";
  }

  //saveImageHDR(imageHDR, numRows, numCols, output_file);
  fromRGBToText(imageHDR, numRows, numCols);
  delete[] imageHDR;
  delete[] h_red;
  delete[] h_green;
  delete[] h_blue;
